...
int main()
{
  ...
  // cria a memória
  mem_t *mem = init_mem();
  // cria dispositivos de E/S (o relógio, terminal e random)
  term_t *term = term_cria();
  rel_t *rel = rel_cria();
  struct rand *random = random_cria();
  ...
  es_registra_dispositivo(es, 3, random, 2, random_gera, NULL);
